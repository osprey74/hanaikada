...
調査結果の詳細と一次情報は `learnings.md`（L1〜L4）を参照。

1. **[解決]** 初回同期の既定遡り期間 → **30 日を暫定既定**として据え置き（設定で 7 / 30 / 90 日）。確定はリリース前に実アカウントで所要時間・件数を計測して行う（Phase 2 の計測タスク）。→ `learnings.md` L3
2. **[解決]** 同一投稿の複数枚をまとめるか、ばらすか → 既定は **「1 タイルにまとめる（枚数バッジ付き）」**、設定でばらし表示に切替可能。design_handoff の確定 UI 仕様に準拠。→ `learnings.md` L4
3. **[解決]** `createSession` のレート制限値 → **アカウント単位で 30 回 / 5 分・300 回 / 日**。`refreshSession` は専用制限なし（IP 単位 3,000 回 / 5 分のグローバル枠）。起動時は必ず `refreshSession` を優先する方針を確定。→ `learnings.md` L1
4. **[解決]** 動画再生の実装方式 → **hls.js を同梱し `Hls.isSupported()` で分岐**する統一実装。macOS の WKWebView はネイティブ HLS 再生可、Windows の WebView2 はネイティブ非対応のため hls.js（MSE 経由）で再生。両 OS でアプリ内再生を主経路とし、外部ブラウザ誘導は最終手段。Windows 実機での動作サンプル確認のみ Phase 4 着手時に実施。→ `learnings.md` L2
