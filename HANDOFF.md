...
- [x] 初回同期で利用可能なタイムライン全体のメディア投稿が DB に入る（2026-08-29 実機確認: 空 DB から初回同期でタイムライン終端まで 170 ページ・メディア 4,995 件を取得。**`getTimeline` の履歴は約 1.5 日・約 5,000 件が上限で「30 日」は原理的に取得不能**なため、cursor レジューム＋上限 2,000 ページで「全ドレイン」する仕様に変更。→ `learnings.md` L3）
- [x] 2 回目以降の同期が既知 URI で早期打ち切りされる（ログで確認）（2026-08-29 実機確認: 差分同期が 1 ページ目で既知 URI に到達し 293ms で打ち切り）
- [x] 429 を人為的に発生させたとき、バックオフして復帰する（実機では枠に余裕があり 429 未発生。`Retry-After` / `RateLimit-Reset` 解釈と指数バックオフ＋上限頭打ちを単体テストで担保。中断可能スリープ経由で復帰）
- [x] 4 枚組の画像投稿が `media` に 4 レコードとして正しく展開される（2026-08-29 実機確認: 4 枚組投稿 30 件を DB に確認。単体テストでも網羅）
- [x] `recordWithMedia`（引用＋画像）が取り込まれ、`record` 単体は除外される（単体テスト＋実データで確認）
- [x] `aspect_w` / `aspect_h` が欠損なく入る（欠損時は 1:1 でフォールバック）（2026-08-29 実機確認: 欠損は 21/801 ≈ 2.6%。UI フォールバックは Phase 3）
- [x] 同期中に UI 操作をしても固まらない（別タスクで実行。同期中に中断ボタン操作・進捗反映が機能することを実機確認）

**Phase 2 で判明した設計変更（実装・確認済み）**

- 初回同期の上限を 30 → **2,000 ページ/回**に引き上げ、`sync_state.cursor` を初回同期専用のレジューム位置として保持（中断/再開で継続）。差分同期は cursor を壊さない（`touch_sync_state`）。
- 実運用では cutoff（日数）より先にタイムライン終端が来るため、初回同期は「利用可能なタイムライン全体をドレイン」する動作になる（本アカウントで約 5,000 件・約 1 分）。詳細と計測値は `learnings.md` L3。
- 定期ポーリング（DESIGN §6.2 の 5 分間隔・非アクティブ時 3 倍）は Phase 2 の受け入れ条件外のため未実装（手動トリガのみ）。Phase 5 または別途で対応。
...
2026-08-30 に 10,000 件規模で検証（実データ 4,999 件を複製し 9,998 件へ増量、実 CDN サムネで負荷）。

- [x] 10,000 件のメディアでスクロールが 60fps を維持する（実機確認: 約 1 万件で高速上下スクロールも滑らかに追従、カクつきなし）
- [x] 画像読み込み前後でタイル位置がずれない（`aspectRatio` プレースホルダで箱を先確定。スクロール中の飛びなしを実機確認）
- [x] フィルタ変更が 200ms 以内に反映される（10k で SQL 実測: 1 ページ目取得の最悪 21.7ms・典型 1〜8ms、`media_count` 3.6ms。ベンチ `db::queries::tests::bench_query_10k`）
- [x] 新着同期時、スクロール位置を保ったまま上部に追加される（実装確認: 最上部なら即反映、スクロール中は「新着 N 件」ピルで位置維持。※タイムライン枯渇のため本セッションでの live 再現はなし。コード＋Phase 2 同期時の挙動で確認）
- [x] 通常利用時のメモリが 500MB を超えない（実機確認: Private Working Set 267→323MB（1万件表示・debug ビルド）。500MB 以下）
...
- [x] `Esc` ですべてのオーバーレイが確実に閉じる（2026-08-29 実機確認: ビューア表示中は GridApp のキー処理を無効化し、ビューア側 `Esc` で閉じる）
- [x] ラベル付きメディアが既定でブラーされ、クリックで解除できる（2026-08-29 実機確認: getPreferences 連携＋既定警告ラベルでブラー、クリック解除・ホバーの「再度隠す」で再ブラー）
- [x] 動画がアプリ内再生できる（2026-08-29 Windows 実機確認: hls.js 経由。**`Hls.isSupported()` 最優先**・**`enableWorker: false`** が必須と判明 → `learnings.md` L2）。macOS ネイティブ経路は Phase 5 のクロス OS ビルド時に確認
- [x] ビューアを閉じた際、グリッドのスクロール位置とフォーカスが保持される（2026-08-29 実機確認: 仮想スクロール位置を維持）
